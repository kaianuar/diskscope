...
| `CRITIC_MODEL` unset / unresolved at JSON-build site → API returns `unsupported_model` → Gate 2 aborts with a cryptic provider error instead of an actionable setup message | M   | H   | `tests/review_gate.sh` MUST apply `${VAR:-<known-good-default>}` at **every** JSON-payload substitution site (not only at env-export in `pipeline.sh`/`run-gates.sh`); MUST pre-flight the resolved model against the `CONFIG.md` allowlist; MUST fail fast with `CRITIC_MODEL unset — set it or update the default in tests/review_gate.sh`. NEVER forward a literal `${CRITIC_MODEL}` / `${PIPELINE_CRITIC_MODEL}` to the provider. |
...
- [ ] **Critic model resolvable** — `tests/review_gate.sh` resolves `CRITIC_MODEL`
      to a non-empty, allowlisted value before any provider call; on unset or
      unsupported model it exits with an actionable setup message
      (no `${CRITIC_MODEL}` literal ever reaches the provider).
