...
impl domain::ports::Trash for TrashBin {
    fn move_to_trash(&self, path: &str) -> Result<(), DomainError> {
        ...
        trash::delete(path).map_err(|e| trash_err(e))?;

        // Now look up the freshly-trashed entry.
        let items_after = os_limited::list().map_err(|e| trash_err(e))?;
        ...
        let trash_item = items_after
            ...
            .ok_or_else(|| {
                DomainError::Io(std::io::Error::new(
                    std::io::ErrorKind::Other,
                    format!("trash entry not found after delete for {path}"),
                ))
            })?;
